...
function ApiRouterCars() {
  ...
  router.put("/:id", upload.single("photo"), async (req, res) => {
    ...
    try {
      ...
      if (req.file) {
        // Jika ada, unggah foto baru ke Cloudinary
        const uniqueFileName = `car_${Date.now()}`;
        const tempFilePath = `/tmp/${uniqueFileName}.jpg`;
        await fs.writeFile(tempFilePath, req.file.buffer);
        ...
        // Perbarui data mobil dengan URL foto baru
        updatedData.photo = result.secure_url;
      }

      // Perbarui data mobil di database
      await db("cars").where("cars_id", "=", id).update(updatedData);

      // Ambil data yang diperbarui
      const data = await db("cars").select("*").where("cars_id", "=", id);
